# Why I'm Looking for New Opportunities

## Current Situation

I was made redundant at Nurtur in February 2026. While this wasn't my choice, it aligns with my career goals and provides an opportunity to pursue the next stage of my professional development.

## Career Trajectory Goals

**Deliberate progression**: I'm building on web development foundations, expanding into AI/ML integration for web applications. This isn't random - it's strategic growth that enhances my core web development strengths.

**My role at Nurtur**: Full Stack Developer with primary focus on React/TypeScript frontend, but with full-stack responsibilities including backend work, architecture decisions, and mentoring.

**Next needed step**: More AI/ML focused work, deeper technical challenges, projects that align with my values of curiosity, collaboration, and craftsmanship.

**Long-term vision**: Becoming a senior technical contributor specializing in AI-powered web applications that solve real problems and create genuine value.

## What I've Achieved at Nurtur (3.5 Years, Part of 5.5 Years Total Professional Experience)

✅ Built production systems (Integrations Dashboard - 3+ years, zero maintenance)  
✅ Contributed to critical infrastructure (Nexus Dashboard, Email Editor)  
✅ Developed mentoring capability (teaching backend devs frontend)  
✅ Proven full-stack ability  
✅ Mastered Redux Toolkit and complex state management  
✅ Learned to work in established codebases at scale

**I've maximized what this role could teach me**. The redundancy provides an opportunity to pursue different challenges that align with my growth trajectory.

## In Canada

**Already relocated**: Based in Halifax, Nova Scotia. IEC Working Holiday visa issued and valid. Available immediately.

**Why Canada**: Natural beauty, work-life balance culture, innovation in technology, strong AI/ML scene. I came to build my life and career here.

**Long-term**: Pursuing Permanent Residency through Canadian Experience Class after one year of Canadian work experience. This isn't about trying out Canada - I made a deliberate choice to be here.

## Seeking Values Alignment

**Current work**: Internal dashboards for business operations. Practical and valuable, but not aligned with my deeper values.

**What I want**: Work on projects that create real impact and solve meaningful problems. AI-powered web applications that enhance user experiences and provide genuine value.

**Examples of interest**:
- AI-powered web applications with intelligent features
- RAG systems and semantic search
- Recommendation engines and personalized experiences
- LLM integration that solves real problems
- Building systems that create tangible value for users

**"I don't want to work for money alone"**: While Nurtur pays well, meaningful work that aligns with my values is more important for long-term satisfaction and sustainability.

## Technical Growth Needs

**Current skills**: Strong in React/TypeScript, Redux, frontend architecture, full-stack web development.

**Skills I'm building**: AI/ML (contextual bandits in WhatNow, LLM integration in moh-ami, RAG in Folio), embedded systems (Atlantis), hardware-software integration.

**What I need next**: Roles that let me apply current web development skills while integrating AI/ML capabilities. Working with experienced engineers on AI-powered web applications, intelligent search systems, or personalized recommendation features.

**Learning style**: I learn best through meaningful projects, not abstract study. I need work that challenges me in directions I want to grow.

## Team and Culture Fit

**What worked at Nurtur**: Small team, professional respect, autonomy to structure my own work.

**What I'm seeking**: 3-4 person team, everyone extremely motivated and believes in the project, everyone has stake in success, decisions by consensus, results over process.

**Leadership opportunity**: Ready to take on more leadership while remaining technical contributor. My "team dad" style brings teams together and encourages everyone's best work.

## Gratitude for the Experience

**Nurtur was exactly what I needed at that stage**: I went from bootcamp graduate to confident production engineer. They gave me mentoring opportunities on the Integrations Dashboard, trusted me as technical lead on critical systems like Nexus.

**Why this transition works**: While the redundancy wasn't my choice, it aligns with my career goals. Nurtur doesn't have AI/ML integration work. They focus on internal business dashboards, not AI-powered web applications. This transition allows me to pursue what I need for the next 5 years of growth.

**Handling the transition professionally**: I'm committed to a smooth transition, documenting my work thoroughly, and helping transition my responsibilities. I respect what they've given me and want to leave them in good shape.

## The Bottom Line

I'm looking because:
1. **Transition**: Being made redundant provides opportunity to pursue next career stage
2. **Geographic**: In Canada (Halifax) — building career and life here long-term
3. **Technical**: Need more AI/ML and systems work to continue growth trajectory
4. **Values**: Want projects that align with curiosity, collaboration, craftsmanship, and helping humanity relate to nature
5. **Timing**: 5.5 years of professional experience is right time to make this move

I'm not running away from problems. I'm running toward my goals. The next role should use my current strengths (full-stack web dev, AI/ML foundations, leadership) while integrating AI capabilities into web applications in a Canadian environment with meaningful work.

