# How do you stay current with technology?

By building things that require new technologies — not studying for its own sake. The skill has to earn its place in something real.

The most recent example is Docker. I set up a home server for streaming and file sharing with friends, and Folio runs on the same box alongside the demo projects it connects to. Docker keeps those services isolated from each other without the complexity of separate machines. I'd used it before in passing, but having a real system I wanted to keep stable forced me to understand it properly.

Portfolio projects are the mechanism: WhatNow required contextual bandits and reinforcement learning. moh-ami needed Next.js 14 App Router, GraphQL, Prisma, and LLM integration. Folio required RAG pipelines, embeddings, and transparent routing in FastAPI rather than leaning on heavyweight orchestration glue. Atlantis required embedded systems and LoRa communication. Each project dragged me into skills I then owned.

I read others' code to understand decisions, not copy patterns. When I needed LLM integration in moh-ami, I studied OpenAI's examples and community approaches — but I wanted to know WHY they made certain choices before adapting them.

Teaching also keeps me honest. Explaining CSS to backend developers or technical systems to sales teams forces me to truly understand what I'm saying. The gaps that teaching reveals get filled.

The progression is deliberate: web development foundations → AI/ML integration → embedded systems. Always building on what's solid, always reaching for what's next.

---

**emotion:** happy
**suggestions:**
- How do you approach learning new tech?
- How do you learn something quickly?
- Do you have experience with Docker?
- Tell me about moh-ami
- Tell me about WhatNow
- What is Folio?

**variants:**
- How do you keep your skills current?
- How do you keep up with new technology?
- How do you stay up to date as a developer?
- How do you learn new technologies while working?
